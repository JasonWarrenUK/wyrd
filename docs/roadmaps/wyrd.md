...
- [ ] **TD.18** — Three-way combine UI for TD.5 divergence — TD.5 shipped detection and markers only; this adds the per-field combine flow: a `huh` form showing user value vs old default vs new default for each diverged entry (from `stage.DetectDiverged`), letting the user adopt individual upstream changes without discarding their own edits. Must not touch `ShadowOf`/`ShadowReason` except on explicit user action — the re-edit-preserves-`ShadowOf` invariant TD.5's tests guard is exactly what a careless combine flow would violate. `TD.18a` resolved the shape: genuine three-way, with the old default recovered from a new **fat shadow** field. Scope, per that decision: (1) **Schema** — `types.Kind` and `types.StageGroup` gain `ShadowSource` (the pre-fork default's content verbatim, `omitempty`), under the same carry-forward-on-re-edit rule `ShadowOf` already follows and excluded from the `hashEntry` content hash for the same reason `ShadowReason` is (introducing it must not invalidate every stamped hash). Stamped at a **subset** of the five `ShadowOf` sites, not all of them: only where `ShadowReason` is `ShadowEdited` or empty (`kind_form.go:364-368`, `stage_form.go:344-348`) — `DetectDiverged` already excludes `ShadowTombstone`, `ShadowRenameFanOut` and `ShadowEditedAndRenamed` entries outright, so those never reach the combine form and stamping them would buy nothing while costing exactly the file noise `TD.18a` weighed as fat shadow's one real drawback. That rules out the tombstone writes (`kind_form.go:397`, `stage_form.go:370`) and both `rename.go` cascade sites (`113`, `155-157`). ⚠️ Breaking change: both JSONC files gain a field, mitigated by `omitempty`; entries stamped before this ships have no `ShadowSource` and must degrade gracefully to a two-way form for that entry rather than erroring. (2) **`DivergedEntry`** gains the resolved old-default content alongside its two hashes. (3) **Scalar fields** (`Kind.Glyph`/`Colour`/`StageGroup`, `StageGroup.Cycle`/`LoopTarget`) — three-way lets the form skip fields the user never touched and label the rest ("upstream changed this" vs "you changed this"); with three mutable fields per type this saves at most two rows, so the value is in the *labelling*, not the row count. (4) **`StageGroup.Stages`** is the real work: per-stage adopt, each upstream-added stage offered individually against the user's list (an addition is only identifiable as one because the old default is now present), each user-only stage offered as keep-or-drop, and every adopted stage followed by an explicit insert-position prompt — anchor-to-neighbour was considered and rejected as too magic for a decision the user should see. (5) Missing-`ShadowSource` entries fall back to a two-way form for that entry only, surfaced explicitly in the form rather than silently _(depends on TD.5, TD.18a)_
...
graph LR
	...
	QC.6 --> MI
	PL.1 --> MJ
	class CO.3,DL.4,NW.1,PL.1,QC.1,QC.2,QC.3,QC.4,QC.5,QC.6,RT.10,RT.6,RT.7,RT.9,SK.1,SL.18,SP.8,SY.3,SY.4,TD.18,TD.22,VP.7,VP.8 todo
